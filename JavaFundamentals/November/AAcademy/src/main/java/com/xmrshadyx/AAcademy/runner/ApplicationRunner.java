...
public class ApplicationRunner implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        Country france = Country.builder().name("France").continent(europe).build();
        Country brazil = Country.builder().name("Brazil").continent(southAmerica).build();
        Country newZealand = Country.builder().name("New Zealand").continent(oceania).build();
        Country southKorea = Country.builder().name("South Korea").continent(asia).build();
        Country canada = Country.builder().name("Canada").continent(northAmerica).build();
        Country noneReal = Country.builder().name("No Real Countries").continent(antarctica).build();
        Country nigeria = Country.builder().name("Nigeria").continent(africa).build();
        ...
        City marseille = City.builder().name("Marseille").city_population(861635L).country(france).build();
        City rioDeJaneiro = City.builder().name("Rio De Janeiro").city_population(6775561L).country(brazil).build();
        City auckland = City.builder().name("Auckland").city_population(1400000L).country(newZealand).build();
        City seoul = City.builder().name("Seoul").city_population(9586195L).country(southKorea).build();
        City toronto = City.builder().name("Toronto").city_population(4263000L).country(canada).build();
        City wholeContinent = City.builder().name("whole Continent").city_population(5000L).country(noneReal).build();
        City lagos = City.builder().name("Lagos").city_population(8048430L).country(nigeria).build();
        ...
        for (City city : cities) {
            String printable = String.format
                    ("In Continent: %s, have a country named: %s where in one of their cities named: %s have population of: %d amount of people.",
                            city.getCountry().getContinent().getName(),city.getCountry().getName(),
                            city.getName(),city.getCity_population());
